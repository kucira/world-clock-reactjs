export const getTimeFormat = (time: Date) => {
  const minutesFormat = time?.getMinutes()?.toString()?.length === 1 ? `0${time?.getMinutes()}` : time?.getMinutes();
  return {
    hours: time?.getHours(),
...
export const getCurrentTime = (dateTime: string) => {
  if(dateTime) {
    ...
    const currentTime = dateTime?.substring(0, indexOffsetPlus > -1 ? indexOffsetPlus : indexOffsetMinus);
...
export const getDiffTimeInLabel = (currentTimeOffset: string, targetTimeOffset: string) => {
  ...
  const diffTime = Math.abs(+currentTimeOffset.substring(1, 3) - +targetTimeOffset.substring(1, 3));
...
export const getDiffTime = (currentDate: Date, targetDate: Date) => {
  ...
  return {
    ...
    label: Math.round(diff) === 0 ? '' : `${Math.abs(Math.round(diff))} hours ${aheadOrBehind}`,
  };
};
